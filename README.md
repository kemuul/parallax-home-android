...
This repository is a working, dependency-light Android Home app written in Kotlin. Install it, choose **Parallax Home** as the Home app, hold the phone at a comfortable angle, tap the transparent menu button to open the controls, tap **RECENTER**, and gently tilt it left or right. The foreground applies a smooth inverse horizontal rotation directly over the unmodified system wallpaper.

Version 0.12 replaces the unreliable swipe gesture with one transparent menu button. Tapping it opens a compact transparent control panel; tapping the close button hides it. The cyan glare, dark gradient, widget-card tint, and pre-Android-12 frosted tint are removed so the system wallpaper keeps its original colors.
...
The system wallpaper is shown directly with no colored atmosphere layer. The foreground is a separate Android `View` layer with:
...
- a dynamic left/right `pivotX`, making the appropriate outer edge act like a book spine;
- no horizontal translation or overscale, so the hinge edge remains fixed while the free edge moves inward.

The effect is GPU-composited by Android; the app does not redraw the whole UI on every sensor event. On Android 12 and newer, a light GPU `RenderEffect` starts only after side tilt exceeds 38 degrees and reaches its readable maximum at 50 degrees. Android 8-11 no longer use a colored fake-blur overlay because it altered wallpaper colors. Tap **PARALLAX EFFECT: OFF** to immediately remove motion and blur; the setting is remembered after restarts.

## 8. Install the APK on a physical phone

### Download a ready-built APK

Open the project's [GitHub Releases page](https://github.com/kemuul/parallax-home-android/releases) on the Android phone, open **Parallax Home v0.12.0 beta**, expand **Assets**, and download `Parallax-Home-v0.12.0-beta.apk`. If Android asks, allow the browser or file manager to **Install unknown apps**, then open the downloaded APK and tap **Install**. After installation, open **Settings -> Apps -> Default apps -> Home app** and select **Parallax Home**.
...
The Home-app control in the transparent panel says **SET AS HOME** before Parallax Home is selected. Once it is the default, the control changes to **CHANGE HOME APP**; tap it and select your original launcher in Android's Home-app settings to switch back. Android deliberately requires the user to approve either choice.
...
3. Tap the transparent menu button at the lower-right. The compact transparent controls should appear above it; tap the close button to hide them again.
...
| `HINGE_SWITCH_DEADBAND_DEGREES` | Prevents the left/right hinge from flickering while nearly centered |
